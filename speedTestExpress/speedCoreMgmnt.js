...
// const decompressTarXz = require('decompress-tarxz');// TODO: COMMENTED due to error accure during installation
...
async function ensureBinary(
  ...
  const binaryLocation =
    'https://bintray.com/ookla/download/download_file?file_path=ookla-speedtest-$v-$p';
  ...
  if (!await fileExists(binPath)) {
    ...
    await decompress(pkgPath, binDir, {
      plugins: [
        ...
        decompressTargz(),
        decompressUnzip(),
        // decompressTarXz(), // TODO: COMMENTED due to error accure during installation
      ],
      filter: file => {
        ...
        if (data.type) {
          ...
          if (content) {
            ...
            if (
              ...
              data.progress =
                priorProgress + progressPhases[data.type] * content.progress;
            }
          }
        } else {
